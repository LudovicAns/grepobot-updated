...
window.ModuleManager = {
  ...
  start: function () {
    ...
    if (nextTimestamp === null && !queueNotEmpty) {
      ConsoleLog.Log(
        GrepoBotCore.t("console.nothing_ready", "Nothing is ready yet!"),
        0,
      );
    ...
    $("#time_grepobot_updated")
      .append(
        FormBuilder.timerBoxSmall({
          id: "Autofarm_timer",
          styles: "",
          text: GrepoBotCore.t("timer.start", "Start GrepoBot Updated"),
        }),
      )
      .show();
  },
  ...
  startTimer: function (intervalSeconds, onComplete) {
    ...
    ModuleManager.interval = setInterval(function () {
      $("#time_grepobot_updated .caption .value_container .curr")["html"](
        GrepoBotCore["toHHMMSS"](intervalSeconds),
      );
  ...
  initButtons: function (moduleName) {
    ...
    toggleEl["on"]("click", function (event) {
      ...
      if (moduleName == "Autoattack" && !GrepoBotCore["checkPremium"]("captain")) {
        HumanMessage["error"](
  ...
  loadModules: function () {
    GrepoBotCore["isLogged"] = true;
    //GrepoBotCore['trial_time'] = _0xa6b2x1e['trial_time'];
    //GrepoBotCore['premium_time'] = _0xa6b2x1e['premium_time'];
    //GrepoBotCore['facebook_like'] = _0xa6b2x1e['facebook_like'];
    //if (_0xa6b2x1e['assistant_settings'] != '') {
    //    Assistant['setSettings'](_0xa6b2x1e['assistant_settings'])
    //};
    /*if (!_0xa6b2x1e['player_email']) {
            GrepoBotCore['verifyEmail']()
        };*/
    //if (GrepoBotCore['trial_time'] - Timestamp['now']() >= 0 || GrepoBotCore['premium_time'] - Timestamp['now']() >= 0) {
    if (
      ...
      $["when"](
        $["ajax"]({
          method: "GET",
          //data: GrepoBotCore['Account'],
          url: GrepoBotCore["domain"] + "Autofarm.js",
          dataType: "script",
        }),
        $["ajax"]({
          method: "GET",
          //data: GrepoBotCore['Account'],
          url: GrepoBotCore["domain"] + "Autoculture.js",
          dataType: "script",
        }),
        $["ajax"]({
          method: "GET",
          //data: GrepoBotCore['Account'],
          url: GrepoBotCore["domain"] + "Autobuild.js",
          dataType: "script",
        }),
        $["ajax"]({
          method: "GET",
          //data: GrepoBotCore['Account'],
          url: GrepoBotCore["domain"] + "Autoattack.js",
          dataType: "script",
        }),
    ...
    /*} else {
            ...
            GrepoBotCore['createNotification']('getPremiumNotification', 'Unfortunately your premium membership is over. Please upgrade now!')
        }*/
  },
  requiredPrem:
